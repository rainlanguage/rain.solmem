...
/// @title LibStack
/// @notice Basic tools for working with a stack.
library LibStack {
    using LibStack for StackItem[];

    /// Pointer to the start (length prefix) of a `StackItem[]`.
    /// @param stack The stack to get the start pointer of.
    /// @return pointer The pointer to the start of `array`.
    function startPointer(StackItem[] memory stack) internal pure returns (Pointer pointer) {
        assembly ("memory-safe") {
            pointer := stack
        }
    }

    /// Pointer to the data of a `StackItem[]` NOT the length prefix.
    /// @param stack The stack to get the data pointer of.
    /// @return pointer The pointer to the data of `array`.
    function dataPointer(StackItem[] memory stack) internal pure returns (Pointer pointer) {
        assembly ("memory-safe") {
            pointer := add(stack, 0x20)
        }
    }

    /// Pointer to the end of the allocated memory of an stack.
    /// @param stack The array to get the end pointer of.
    /// @return pointer The pointer to the end of `stack`.
    function endPointer(StackItem[] memory stack) internal pure returns (Pointer pointer) {
        assembly ("memory-safe") {
            pointer := add(stack, add(0x20, mul(0x20, mload(stack))))
        }
    }

    /// Cast a `Pointer` to `StackItem[]` without modification or safety checks.
    /// The caller MUST ensure the pointer is to a valid region of memory for
    /// some `StackItem[]`.
    /// @param pointer The pointer to cast to `StackItem[]`.
    /// @return stack The cast `StackItem[]`.
    function unsafeAsStack(Pointer pointer) internal pure returns (StackItem[] memory stack) {
        assembly ("memory-safe") {
            stack := pointer
        }
    }

    /// Building stacks from literal components is a common task that introduces
    /// boilerplate that is either inefficient or error prone.
    /// @param a A single integer to build a stack around.
    /// @return stack The newly allocated stack including `a` as a single item.
    function stackFrom(StackItem a) internal pure returns (StackItem[] memory stack) {
        assembly ("memory-safe") {
            stack := mload(0x40)
            mstore(stack, 1)
            mstore(add(stack, 0x20), a)
            mstore(0x40, add(stack, 0x40))
        }
    }

    /// Building stacks from literal components is a common task that introduces
    /// boilerplate that is either inefficient or error prone.
    /// @param a The first integer to build a stack around.
    /// @param b The second integer to build a stack around.
    /// @return stack The newly allocated stack including `a` and `b` as the only
    /// items.
    function stackFrom(StackItem a, StackItem b) internal pure returns (StackItem[] memory stack) {
        assembly ("memory-safe") {
            stack := mload(0x40)
            mstore(stack, 2)
            mstore(add(stack, 0x20), a)
            mstore(add(stack, 0x40), b)
            mstore(0x40, add(stack, 0x60))
        }
    }
    ...
    /// @param a The first integer to build a stack around.
    /// @param b The second integer to build a stack around.
    /// @param c The third integer to build a stack around.
    /// @return stack The newly allocated stack including `a`, `b` and `c` as the
    /// only items.
    function stackFrom(StackItem a, StackItem b, StackItem c) internal pure returns (StackItem[] memory stack) {
        assembly ("memory-safe") {
            stack := mload(0x40)
            mstore(stack, 3)
            mstore(add(stack, 0x20), a)
            mstore(add(stack, 0x40), b)
            mstore(add(stack, 0x60), c)
            mstore(0x40, add(stack, 0x80))
        }
    }
    ...
    /// boilerplate that is either inefficient or error prone.
    /// @param a The first integer to build a stack around.
    /// @param b The second integer to build an stack around.
    /// @param c The third integer to build an stack around.
    /// @param d The fourth integer to build an stack around.
    /// @return stack The newly allocated stack including `a`, `b`, `c` and `d` as the
    /// only items.
    function stackFrom(StackItem a, StackItem b, StackItem c, StackItem d)
        internal
        pure
        returns (StackItem[] memory stack)
    {
        assembly ("memory-safe") {
            stack := mload(0x40)
            mstore(stack, 4)
            ...
            mstore(add(stack, 0x40), b)
            mstore(add(stack, 0x60), c)
            mstore(add(stack, 0x80), d)
            mstore(0x40, add(stack, 0xA0))
        }
    }
    ...
    /// @param b The second integer to build a stack around.
    /// @param c The third integer to build a stack around.
    /// @param d The fourth integer to build a stack around.
    /// @param e The fifth integer to build a stack around.
    /// @return stack The newly allocated stack including `a`, `b`, `c`, `d` and
    /// `e` as the only items.
    function stackFrom(StackItem a, StackItem b, StackItem c, StackItem d, StackItem e)
        internal
        pure
        returns (StackItem[] memory stack)
    {
        assembly ("memory-safe") {
            ...
            mstore(add(stack, 0x60), c)
            mstore(add(stack, 0x80), d)
            mstore(add(stack, 0xA0), e)
            mstore(0x40, add(stack, 0xC0))
        }
    }
    ...
    function stackFrom(bytes32 a, bytes32 b, bytes32 c, bytes32 d, bytes32 e, bytes32 f)
        internal
        pure
